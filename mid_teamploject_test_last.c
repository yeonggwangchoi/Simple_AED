...
unsigned char sec_cnt=0, t_sec=0, t_min=0; //타이머 셋팅을 위한 변수
...
// Place your code here
    ...
    if(cpr_count%30==0) ////cpr횟수가 30번 지나면 충전이 완료 
    {
...
void first_system_on(void){
    ...
    delay_ms(1000);
    lcd_clear();
    lcd_putsf("adjust the"); //충전 볼트를 조절하세요
    lcd_gotoxy(0,1);
    lcd_putsf("Charging volt");  
    delay_ms(2000);
...
void cpr(void){ 
    ...
    delay_ms(1000);
    lcd_clear();
    lcd_putsf("Charge for"); //전기충격을 위해 충전합니다.
    lcd_gotoxy(0,1);
    lcd_putsf("electric shock"); 
    delay_ms(2000);
    lcd_clear();
